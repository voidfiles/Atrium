 ...
 */
define('MDFY_LINKS_EACH_PARAGRAPH', true);
...
class Markdownify {
  ...
  function handleTag_code() {
    ...
    if ($this->parser->isStartTag) {
      ...
      if (strpos($buffer, "\n") !== FALSE) {
        if (substr($buffer, -1) == "\n") {
          //don't add spaces to last newline
          $buffer = str_replace("\n","\n    ",substr($buffer, 0, strlen($buffer)-1)) . "\n";
        } else {
          // the code block includes a newline so we'd rather do it by indentation
          $buffer = str_replace("\n","\n    ",$buffer);
        }
        $buffer = $this->decode($buffer);
        $this->out($buffer, true);
      } else {
        //DG hacked to remove unnecessary encoding in code blocks
        $buffer = $this->decode($buffer);
